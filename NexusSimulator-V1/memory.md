...
- `src/headless-editor-adapter.js` maps the Nexus Engine Core Headless Editor's nine-stage lifecycle onto one typed NexusSimulator operation. `src/headless-editor-runner.js` discovers Nexus Engine, injects the adapter into a file-backed harness, and stores every run under ignored local evidence.
- `headless run` is the direct Engine-to-Simulator integration boundary. It adds no HTTP server, subprocess RPC, queue, or Engine dependency on NexusSimulator; invalid actions, events, and procedural settings stop during validation before a SimSpace or WorldFactory render is created. SimSpace source SHA-256 digests are captured before and after execution and must match during verification.
...
- `scene.agent-showcase --prompt <text>` routes descriptions through `src/world-domain-planner.js`. The planner builds semantic domains, requirements, capability ownership, composition edges, ambiguities, gaps, and a complete coverage ledger before materializing any object steps. It accepts a validated agent-authored plan or uses a deterministic fallback; unsupported intent fails closed instead of selecting an unrelated preset. The planner owns what belongs, deterministic factories own how typed settings and a seed generate it, and the reviewing agent owns revision and composition.
- `world-domain capabilities` exposes the executable capability inventory with semantic terms, biome affinity, review criteria, and typed setting schemas. `world-domain plan` exposes the non-mutating semantic plan and accepts `--agent-plan` for an independently coverage-checked `nexus.world-domain-plan.v1` input. Prompt runs retain both domain-plan and world-plan digests, disable template sector replication, use local showcase terrain when NexusEngine roots are absent, and retain the TerrainKit/ProtoKit gate when both roots are configured.
- `world-video` is the local short-form review lane around `scene.agent-showcase`. It scales the proof timeline to 15 seconds, deterministically captures native 24 FPS gameplay frames in a 720x1280 browser presentation, derives a separate 4 FPS harness view by selecting every sixth frame without interpolation, opens on the completed world, and shows typed settings rebuilding the same placed candidate in context.
- `.nexus-simulator/world-video-loop/` stores the current full gameplay draft and compact 4 FPS harness-view proxy plus per-iteration manifests, posters, contact sheets, append-only reviews, and an issue/change/evidence graph. Technical review rejects mismatched capture/output cadence and frame counts. A passing replacement removes the prior full video and temporary showcase run while retaining its hash and review history. Raw drafts are never publication-authorized.
- `world-video` keeps `currentIterationId` separate from `latestAttemptIterationId`; failed or interrupted captures are graphed and compacted but cannot replace the retained reviewable MP4.
- Headless WorldFactory operations keep generation, human review, and revision in separate runs. Revisions preserve prompt, seed, and viewport, apply typed settings patches to selected procedural objects, and must produce a different artifact hash before replacing the retained candidate.
- World-video graph evidence includes semantic domains, requirements, selected factory capabilities, composition relationships, and one saturation experiment per iteration. Coverage completeness does not imply visual acceptance; saturation remains held until structural gates and human review pass, and repeated composition signatures are excluded.
- Every `scene.agent-showcase` world run creates a new run-local blank Nexus Engine project and rejects reused non-empty run IDs. Generation does not consume prior-run lessons, geometry, textures, or world state. Each fresh kit must pass isolated Core World, Object, Graphics, Physics, fidelity, portability, and compilation checks before a second fresh engine composes all passing kits. Natural-math candidates come from a ranked source-backed catalog; low-poly geometry plus procedural principled-PBR reskinning is the default unless the prompt explicitly overrides it.
...
- Prompt-built worlds must render only declared domain requirements and native support systems. Template props, paths, validation monuments, replicated sectors, and showroom stages cannot leak into an agent-planned composition unless the plan owns them.
- Operator repairs preserve prompt, seed, candidate identity, and accepted world state. Prefer a typed settings or placement patch; change factory code only when the public controls cannot express the correction.
...
- Preserve seeds while repairing one video issue; use a new seed when advancing to a different place so graph evidence isolates factory changes from world variation.
...
- Nexus Engine remains the control plane and NexusSimulator remains an independently usable execution and evidence service. MCP may wrap the proven runner later but does not replace this direct adapter boundary.
